...
async function safeQuery(tableName, selectFields = '*', filters = {}) {
  try {
    ...
    if (filters.company && filters.company !== 'All') {
      query = query.or(`company.eq.${filters.company},company_name.eq.${filters.company}`)
    }
    if (filters.location && filters.location !== 'All') {
      query = query.or(`location.eq.${filters.location},location_name.eq.${filters.location}`)
    }
...
export async function getDashboardData(company = 'All', location = 'All', year = 'All') {
  ...
  try {
    ...
    const sifFromGoodCatch = goodCatchData.filter(g => g.sif_potential === true || g.is_sif_p === true).length
    const sifFromIncidents = incidentsData.filter(i => i.is_sif === true || i.is_sif_p === true || i.psif_classification?.includes('SIF') || i.psif_classification?.includes('PSIF')).length
    const sifFromHazardId = hazardIdData.filter(h => h.sif_potential === true || h.threat_level === 'High').length
    const sifFromBBS = bbsData.filter(b => b.sif_potential === true).length
    ...
    // Direct control status from incidents
    const directControlEffective = incidentsData.filter(i => i.direct_control_status === 'Effective').length
    const directControlFailed = incidentsData.filter(i => i.direct_control_status === 'Failed').length
    const directControlAltOnly = incidentsData.filter(i => i.direct_control_status === 'Alternative Only').length
    const directControlNone = incidentsData.filter(i => i.direct_control_status === 'None' || !i.direct_control_status).length
    ...
    thaData.forEach(tha => {
      if (tha.energy_types && Array.isArray(tha.energy_types)) {
        tha.energy_types.forEach(e => {
      ...
      energyTypes.forEach(type => {
        if (tha[`energy_${type.toLowerCase()}`] === true) byEnergyType[type]++
      })
    })

    // Count from STOP Take 5
    stopTake5Data.forEach(st => {
      if (st.energy_types && Array.isArray(st.energy_types)) {
        st.energy_types.forEach(e => {
          const normalized = energyTypes.find(t => e?.toLowerCase().includes(t.toLowerCase()))
          if (normalized) byEnergyType[normalized]++
        })
      }
      energyTypes.forEach(type => {
        if (st[`energy_${type.toLowerCase()}`] === true) byEnergyType[type]++
      })
    })

    // Count from Risk Control Conversations
    riskConversationData.forEach(rc => {
      if (rc.energy_types && Array.isArray(rc.energy_types)) {
        rc.energy_types.forEach(e => {
          const normalized = energyTypes.find(t => e?.toLowerCase().includes(t.toLowerCase()))
          if (normalized) byEnergyType[normalized]++
        })
      }
