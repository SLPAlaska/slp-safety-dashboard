...
function calculateAreasNeedingFocus({
  ...
  // ========================================================================
  // 1. LSR AUDIT ISSUES (existing logic - enhanced)
  // ========================================================================
  const allLsrAudits = [
  ...
  allLsrAudits.forEach(audit => {
    ...
    if (hasIssue) {
      ...
      if (!lsrIssuesByCategory[key]) {
        lsrIssuesByCategory[key] = { count: 0, issues: [], locations: {} }
      }
      lsrIssuesByCategory[key].count++
      if (audit.findings) lsrIssuesByCategory[key].issues.push(audit.findings)
      const loc = audit.location || audit.location_name || 'Unknown'
      lsrIssuesByCategory[key].locations[loc] = (lsrIssuesByCategory[key].locations[loc] || 0) + 1
    }
  })

  Object.entries(lsrIssuesByCategory).forEach(([category, data]) => {
    focusAreas.push({
      source: 'LSR Audit',
      category,
      ...
      priority: data.count * 10 // Base priority for LSR issues
    })
  })
  ...
  // ========================================================================
  // 7. INSPECTION FAILURES
  // ========================================================================
  const inspectionTypes = [
    { data: fireExtData, name: 'Fire Extinguisher', failField: ['pass', 'satisfactory', 'compliant'] },
    { data: eyewashData, name: 'Eyewash Station', failField: ['pass', 'satisfactory', 'compliant'] },
    { data: firstAidData, name: 'First Aid Kit', failField: ['pass', 'satisfactory', 'compliant'] },
    { data: aedData, name: 'AED', failField: ['pass', 'satisfactory', 'compliant'] },
    { data: ladderData, name: 'Ladder', failField: ['pass', 'satisfactory', 'compliant'] },
    { data: harnessData, name: 'Harness', failField: ['pass', 'satisfactory', 'compliant'] },
    { data: lanyardData, name: 'Lanyard/SRL', failField: ['pass', 'satisfactory', 'compliant'] },
    { data: vehicleData, name: 'Vehicle', failField: ['pass', 'satisfactory', 'compliant'] },
    { data: forkliftInspData, name: 'Forklift', failField: ['pass', 'satisfactory', 'compliant'] },
    { data: craneInspData, name: 'Crane', failField: ['pass', 'satisfactory', 'compliant'] },
    { data: heavyEquipData, name: 'Heavy Equipment', failField: ['pass', 'satisfactory', 'compliant'] },
    { data: scaffoldData, name: 'Scaffold', failField: ['pass', 'satisfactory', 'compliant'] },
  ]

  inspectionTypes.forEach(({ data, name, failField }) => {
    if (!data) return
    
    const failures = data.filter(insp => {
      return failField.some(field => 
        insp[field] === false || 
        insp[field] === 'No' || 
        insp[field] === 'Fail' ||
        insp.deficiencies_found === true ||
        insp.action_required === true
      )
    })
    ...
    if (failures.length >= 1) {
      ...
      focusAreas.push({
        ...
        issue: `${failures.length} failed inspection(s)`,
