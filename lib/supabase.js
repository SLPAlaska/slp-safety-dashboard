...
export async function getDashboardData(company = 'All', location = 'All', year = 'All') {
  ...
  try {
    ...
    const totalSifEvents = sifFromGoodCatch + sifFromIncidents + sifFromHazardId + sifFromBBS + sifFromPropertyDamage
    // Include ALL sources in denominator that are counted in numerator (including BBS)
    const totalEventsForSif = goodCatchData.length + incidentsData.length + hazardIdData.length + propertyDamageData.length + bbsData.length
