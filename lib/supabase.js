...
const supabaseKey = (typeof window === 'undefined' && process.env.SUPABASE_SERVICE_ROLE_KEY) || process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Iml5cGV6aXJ3ZGxxcHB0anBlZXlmIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Njg2Nzg3NzYsImV4cCI6MjA4NDI1NDc3Nn0.rfTN8fi9rd6o5rX-scAg9I1BbC-UjM8WoWEXDbrYJD4'
...
async function safeQuery(tableName, selectFields = '*', filters = {}) {
  try {
    ...
    // DEBUG: Log inspection and LSR query results
    if (tableName.includes('lsr_') || tableName.includes('inspection')) {
      console.log(`[DEBUG] ${tableName}: SQL returned ${(data || []).length}, after JS filter: ${results.length}, filters:`, JSON.stringify(filters))
    }
    return results
  } catch (e) {
...
function calculateAreasNeedingFocus({
  // LSR Audits (7 tables — fall protection includes heights, work permits includes hot work)
  lsrConfinedData,
  lsrDrivingData,
  lsrEnergyData,
  lsrFallData,
  ...
  // 10. TANK FARM INSPECTION DEFICIENCIES (18 AAC 75.075)
  const tankDeficiencies = (weeklyTankInspData || []).filter(ti =>
    ti.corrective_actions_needed === 'Yes' ||
    ti.overall_result === 'DEFICIENCIES FOUND'
  )

  if (tankDeficiencies.length > 0) {
    const tankLocations = {}
    const tankFarms = {}
    let sheenEvents = 0

    tankDeficiencies.forEach(ti => {
      const loc = ti.location || 'Unknown'
      tankLocations[loc] = (tankLocations[loc] || 0) + 1
      const farm = ti.tank_farm_id || 'Unknown'
      tankFarms[farm] = (tankFarms[farm] || 0) + 1
      if (ti.sheen_present_on_drain === 'Yes - Sheen Present') sheenEvents++
    })

    focusAreas.push({
      source: 'Tank Inspection',
      category: 'Containment Deficiency',
      issue: `${tankDeficiencies.length} tank inspection(s) with deficiencies requiring corrective action`,
      count: tankDeficiencies.length,
      severity: sheenEvents > 0 ? 'high' : tankDeficiencies.length >= 3 ? 'high' : tankDeficiencies.length >= 2 ? 'medium' : 'low',
      topLocation: Object.entries(tankLocations).sort((a, b) => b[1] - a[1])[0]?.[0] || 'N/A',
      priority: (tankDeficiencies.length * 15) + (sheenEvents * 40)
    })

    // Sheen events are critical ADEC regulatory triggers — always surface separately
    if (sheenEvents > 0) {
      focusAreas.push({
        source: 'Tank Inspection',
        category: 'ADEC Sheen Event',
        issue: `${sheenEvents} sheen event(s) detected during containment drainage — ADEC reportable`,
        count: sheenEvents,
        severity: 'high',
        topLocation: Object.entries(tankLocations).sort((a, b) => b[1] - a[1])[0]?.[0] || 'N/A',
        priority: sheenEvents * 50 // Highest priority — regulatory trigger
      })
    }
  }
...
export async function getDashboardData(company = 'All', location = 'All', year = 'All') {
  ...
  try {
    ...
    const [
      ...
      // LSR Audits (7 tables)
      // lsr_fall_protection includes working at heights
      // lsr_work_permits includes hot work permits
      lsrConfinedData,
      lsrDrivingData,
      lsrEnergyData,
      lsrFallData,
      ...
      // Tank Farm Inspections (18 AAC 75.075)
      weeklyTankInspData,
      ...
      safeQuery('tha_assessments', '*', filters),
      ...
      // LSR Audits (table names end in _audits, match actual Supabase schema)
      // NOTE: lsr_work_permits_audits covers ALL permit types including Hot Work permits
      // NOTE: lsr_fall_protection_audits covers both fall protection and working at heights
      safeQuery('lsr_confined_space_audits', '*', { ...filters, dateField: 'date' }),
      safeQuery('lsr_driving_audits', '*', { ...filters, dateField: 'date' }),
      safeQuery('lsr_energy_isolation_audits', '*', { ...filters, dateField: 'date' }),
      safeQuery('lsr_fall_protection_audits', '*', { ...filters, dateField: 'date' }),
      safeQuery('lsr_lifting_operations_audits', '*', { ...filters, dateField: 'date' }),
      safeQuery('lsr_line_of_fire_audits', '*', { ...filters, dateField: 'date' }),
      safeQuery('lsr_work_permits_audits', '*', { ...filters, dateField: 'date' }),
      
      // Monthly Inspections (use 'date' field, not 'created_at', for year filtering)
      safeQuery('fire_extinguisher_inspections', '*', { ...filters, dateField: 'date' }),
      safeQuery('eyewash_station_inspections', '*', { ...filters, dateField: 'date' }),
      safeQuery('first_aid_kit_inspections', '*', { ...filters, dateField: 'date' }),
      safeQuery('aed_inspections', '*', { ...filters, dateField: 'date' }),
      safeQuery('ladder_inspections', '*', { ...filters, dateField: 'date' }),
      safeQuery('harness_inspections', '*', { ...filters, dateField: 'date' }),
      safeQuery('lanyard_srl_inspections', '*', { ...filters, dateField: 'date' }),
      safeQuery('shackle_inspections', '*', { ...filters, dateField: 'date' }),
      safeQuery('synthetic_sling_inspections', '*', { ...filters, dateField: 'date' }),
      safeQuery('wire_rope_inspections', '*', { ...filters, dateField: 'date' }),
      safeQuery('chain_hoist_inspections', '*', { ...filters, dateField: 'date' }),
      safeQuery('emergency_drill_evaluations', '*', { ...filters, dateField: 'date' }),
      
      // Equipment (use 'date' field for year filtering)
      safeQuery('vehicle_inspections', '*', { ...filters, dateField: 'date' }),
      safeQuery('forklift_inspections', '*', { ...filters, dateField: 'date' }),
      safeQuery('crane_inspections', '*', { ...filters, dateField: 'date' }),
      safeQuery('heavy_equipment_inspections', '*', { ...filters, dateField: 'date' }),
      safeQuery('scaffold_inspections', '*', { ...filters, dateField: 'date' }),
      
      // Investigation
      safeQuery('investigation_corrective_actions', '*', filters),
      safeQuery('incident_costs', '*', filters),
      
      // Tank Farm Inspections (ADEC 18 AAC 75.075)
      safeQuery('weekly_tank_inspections', '*', { ...filters, dateField: 'date' }),
    ])

    // Merge THA submissions (JotForm) + THA assessments (Portal) into one combined array
    const thaData = [...thaSubmissionsData, ...thaAssessmentsData]
    ...
    // Tank Farm Inspections with deficiencies needing corrective action
    weeklyTankInspData.filter(ti => 
      ti.corrective_actions_needed === 'Yes' || 
      ti.overall_result === 'DEFICIENCIES FOUND'
    ).forEach(ti => {
      const created = new Date(ti.date || ti.created_at)
      const daysOpen = Math.floor((today - created) / (1000 * 60 * 60 * 24))
      openItems.push({
        form: 'Tank Inspection',
        id: ti.inspection_id || ti.id,
        company: ti.company || 'N/A',
        location: ti.location || 'N/A',
        status: ti.overall_result || 'Deficiency',
        daysOpen,
        created: ti.date || ti.created_at
      })
    })
    ...
    const inspectionCounts = {
      ...
      tankFarm: weeklyTankInspData.length
    }
    ...
    const allDataArrays = [
      ...
      toolboxMeetingsData, hseContactsData, incidentsData, weeklyTankInspData
    ]
    ...
    let trainingMetrics = { completions: 0, workers: 0, last30: 0 }
    try {
      let tq = supabase
        .from('lms_completions')
        .select('completed_at, lms_users!inner(full_name, lms_companies!inner(name))')
      if (year && year !== 'All') {
        tq = tq.gte('completed_at', `${year}-01-01`).lte('completed_at', `${year}-12-31`)
      }
      const { data: tc, error: tErr } = await tq
      if (tErr) throw tErr
      let tRows = tc || []
      if (company && company !== 'All') {
        tRows = tRows.filter(r => r.lms_users?.lms_companies?.name === company)
      }
      const cutoff30 = Date.now() - 30 * 86400000
      trainingMetrics = {
        completions: tRows.length,
        workers: new Set(tRows.map(r => r.lms_users?.full_name).filter(Boolean)).size,
        last30: tRows.filter(r => r.completed_at && new Date(r.completed_at).getTime() > cutoff30).length
      }
    } catch (e) { console.warn('Training metrics unavailable:', e?.message) }
    ...
    return {
      ...
      inspectionCounts,
      tankFarmMetrics: (() => {
        const deficiencyInspections = weeklyTankInspData.filter(ti => 
          ti.overall_result === 'DEFICIENCIES FOUND' || ti.corrective_actions_needed === 'Yes'
        )
        const sheenEvents = weeklyTankInspData.filter(ti => 
          ti.sheen_present_on_drain === 'Yes - Sheen Present'
        )
        const totalDeficiencies = weeklyTankInspData.reduce((sum, ti) => sum + (ti.deficiency_count || 0), 0)
        const totalItemsInspected = weeklyTankInspData.reduce((sum, ti) => sum + (ti.items_inspected || 0), 0)
        const uniqueFarms = [...new Set(weeklyTankInspData.map(ti => ti.tank_farm_id).filter(Boolean))]
        
        return {
          totalInspections: weeklyTankInspData.length,
          inspectionsWithDeficiencies: deficiencyInspections.length,
          deficiencyRate: totalItemsInspected > 0 
            ? Math.round((totalDeficiencies / totalItemsInspected) * 1000) / 10 
            : 0,
          totalDeficiencies,
          sheenEvents: sheenEvents.length,
          uniqueTankFarms: uniqueFarms.length,
          tankFarmIds: uniqueFarms,
          satisfactoryRate: weeklyTankInspData.length > 0 
            ? Math.round(((weeklyTankInspData.length - deficiencyInspections.length) / weeklyTankInspData.length) * 100) 
            : 0
        }
      })(),
      lsrAuditCounts: {
        ...
        fallProtection: lsrFallData.length,        // includes working at heights
        liftingOperations: lsrLiftingData.length,
        lineOfFire: lsrLineOfFireData.length,
        workPermits: lsrWorkPermitsData.length,     // includes hot work permits
