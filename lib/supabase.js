...
export async function getDashboardData(company = 'All', location = 'All', year = 'All') {
  ...
  try {
    ...
    const [
      ...
      // LSR Audits (table names match Supabase schema)
      // NOTE: lsr_work_permits covers ALL permit types including Hot Work permits
      // NOTE: lsr_fall_protection covers both fall protection and working at heights
      safeQuery('lsr_confined_space', '*', filters),
      safeQuery('lsr_driving', '*', filters),
      safeQuery('lsr_energy_isolation', '*', filters),
      safeQuery('lsr_fall_protection', '*', filters),
      safeQuery('lsr_lifting_ops', '*', filters),
      safeQuery('lsr_line_of_fire', '*', filters),
      safeQuery('lsr_work_permits', '*', filters),
